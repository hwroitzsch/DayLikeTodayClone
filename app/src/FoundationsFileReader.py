			...
			for index, line in enumerate(opened_file):
				if True:
					...
					if len(line_parts) == self.attribute_count:
						...
						line_attributes['type'] = ''
						line_attributes['revenue'] = ''
						...
						if line_parts[1] != '':
							line_attributes['name'] = line_parts[1].split('"', -1)[1]
						if line_parts[2] != '':
							line_attributes['start_date'] = line_parts[2].split('"', -1)[1]
						...
						if line_parts[3] != '':
							line_attributes['type'] = line_parts[3].strip('<>').split('/', -1)[-1]
						if line_parts[4] != '':
							line_attributes['revenue'] = line_parts[4]
						if line_parts[5] != '':
							line_attributes['image_url'] = line_parts[5].strip('<>')
						if line_parts[6] != '':
							# print('PART #', index, ': ',  line_parts[6], sep='')
							line_attributes['summary'] = line_parts[6].split('"', -1)[1]
